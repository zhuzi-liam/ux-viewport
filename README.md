...
- 默认预置：`1728 × 1117`、`1440 × 900`、`1280 × 800`、`768 × 900`
- 一键应用默认尺寸，初始默认为 `1440 × 900`
- 所有预置均可编辑、删除、设为默认
- 自定义尺寸可仅应用，也可保存后应用
- 悬浮胶囊实时显示当前网页 viewport，支持收起、关闭和拖拽，松手后自动吸附左右边缘
- 自动退出最大化/全屏并最多校准三次
...
1. 打开要验收的普通网页。
2. 点击浏览器工具栏中的 UX Viewport 图标。
3. 点击主按钮应用默认尺寸，或展开下拉菜单选择其他尺寸。
4. 点击页面其他区域后，组件会收起为实时尺寸胶囊。

## 权限说明

- `activeTab`：只在用户主动点击扩展后访问当前页面。
- `scripting`：向当前页面注入悬浮控件。
- `storage`：在本地保存预置尺寸、默认项和当前标签页的控件位置。

扩展不申请全部网站访问权限，不上传任何数据。

## 已知限制

- `chrome://`、`arc://`、Chrome Web Store 和其他浏览器受限页面不允许注入扩展脚本。
- 跨域导航后 `activeTab` 授权会失效，需要再次点击扩展图标。
